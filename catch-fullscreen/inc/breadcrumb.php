...
if ( ! function_exists( 'catch_fullscreen_breadcrumb' ) ) :
	function catch_fullscreen_breadcrumb() {
		$show_on_home = get_theme_mod( 'catch_fullscreen_breadcrumb_on_homepage' );

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'catch-fullscreen' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'catch-fullscreen' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'catch-fullscreen' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'catch-fullscreen' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'catch-fullscreen' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'catch-fullscreen' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-fullscreen' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-fullscreen' ) ) ) ) ; // WPCS: XSS OK.


				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'catch-fullscreen' ) ), get_the_time( __( 'n', 'catch-fullscreen' ) ) ) ), esc_html( get_the_time( __( 'F', 'catch-fullscreen' ) ) ) ); // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'd', 'catch-fullscreen' ) ) ) . $after; // WPCS: XSS OK.

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-fullscreen' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-fullscreen' ) ) ) ) ; // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'F', 'catch-fullscreen' ) ) ) . $after; // WPCS: XSS OK.


			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'catch-fullscreen' ) ) ) . $after; // WPCS: XSS OK.
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'catch-fullscreen' ), absint( max( $paged, $page ) ) );
